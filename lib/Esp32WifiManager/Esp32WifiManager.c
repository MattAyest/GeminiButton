...
void wifi_manager_init_station(const wifi_manager_config_t *config) {
  ...
  // checks to see if the nvs has an error and if recoverable just rewrite over
  // it
  ...
  wifi_config_t wifi_config = {
      .sta =
          {
              // swapping to version where not hard coded values
              .ssid = config->ssid,
...
static void handle_sta_got_ip(void *event_data) {
  ...
  ESP_LOGI(TAG, "Handler: IP_EVENT_STA_GOT_IP. Got IP:" IPSTR,
...
static void handle_sta_disconnected() {
  ...
  if (s_retry_num < WIFI_MAXIMUM_RETRY) {
    ...
    ESP_LOGI(
        TAG,
        "Handler: WIFI_EVENT_STA_DISCONNECTED. Retrying connection (%d/%d)...",
        s_retry_num, WIFI_MAXIMUM_RETRY);
    esp_wifi_connect();
  } else { // throw error
    ESP_LOGE(TAG,
             "Handler: WIFI_EVENT_STA_DISCONNECTED. Failed to connect after %d "
             "attempts.",
             WIFI_MAXIMUM_RETRY);
    xEventGroupSetBits(s_wifi_event_group,
                       WIFI_FAIL_BIT); // Signal total failure
  }
}
