...
void setUp(void) {
    ...
    Memory_Handler = Pool_Ini(Small_Pool_Size,Medium_Pool_Size,Large_Pool_Size);//small number of blocks so works on all systems
    TEST_ASSERT_NOT_NULL(Memory_Handler);
}
void tearDown(void) {
    Pool_Destroy(Memory_Handler);
}
...
void test_small_double_free(){
    helper_double_free(SMALL_BLOCK_SIZE);
}
void test_medium_double_free(){
    helper_double_free(MEDIUM_BLOCK_SIZE);
}
void test_large_double_free(){
    helper_double_free(LARGE_BLOCK_SIZE);
}
void test_Interleaved_Small_Allocation_And_Free(){
...
void helper_Small_Pool_Exhaustion(size_t Block_Size, size_t Pool_Size){
    ...
    for (size_t i = 0; i < Pool_Size; i++) {
        ...
        TEST_ASSERT_NOT_NULL_MESSAGE(Block_Adress_Pointer[i], "Allocation failed unexpectedly before pool was full.");
    }
    
    TEST_ASSERT_NULL_MESSAGE(Pool_Alloc(Block_Size,Memory_Handler), "Allocation succeeded unexpectedly when pool should be full.");
...
void helper_Confirm_valid_Pool_Creation(size_t Memory_Size, size_t Block_Size){
    ...
    TEST_ASSERT_NOT_NULL_MESSAGE(Block_Adress_Pointer,  "Allocation failed unexpectedly before pool was full.");
